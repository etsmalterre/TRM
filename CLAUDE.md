...
- **Le bonnetier voit moins que le régleur (2026-09-22)** : pas de « Lancement OF »
  (`actionsFor()` API + `actions.ts` rendent `[]`, bande « en attente du régleur », et le
  métier est **inactif** pour lui — `estInactif`, tuile sans barre), pas d'icône Historique, pas d'onglet Notes sur l'écran Consigne. **Le % de 2nd choix est
...
- **`@/` imports inside a shared screen resolve to THIS app's src** — the screen uses TRM's local copies of components/lib/hooks. Keep those copies in sync with ETM (the only intended deltas are in `lib/api.ts`: the `API_URL` dev fallback and `err.body` on failed calls; anything else is drift — `popover-select.tsx` sat one bug fix behind ETM for a month and that was ticket #1160). Verbatim mirrors of ETM files, copied when a screen needed them — improve them **in ETM** and re-copy, never fork: `components/email/SendEmailDialog.tsx`, `components/ui/signature-preview.tsx`, `lib/email.ts`, plus the `components/ui/*`, `lib/*` and `hooks/*` that predate them.
...
- **A per-app difference is a PROP, never a fork.** Three precedents, in increasing strength: `RapportFinance basePath` (a string), `TombeMetierReferences obsOfEditor` (a whole **component** TRM injects, so the shared file never learns a TRM endpoint — see « Observations régleur »), and on the backend `createFinanceRouter(scope)`. Default the prop to the ETM behaviour and the other app is untouched. ⚠️ A shared screen's lookup over an `IDsociete`-partitioned table (e.g. `client`) is ETM's ledger by default — TRM passes its own path (`TombeMetierReferences clientsLookupPath`, LIVA #1201: the Client combo listed ETM clients).
