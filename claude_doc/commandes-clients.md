...
- La pastille « 37 % » de la fiche ligne du legacy est la **marge** : `(prix − PrixDeRevientTRM) / prix`. Elle vit dans `ETM/apps/api/src/lib/pricing-trm.ts`, comme le calcul du tarif suggéré — mais ce sont deux fonctions distinctes, pas interchangeables.
  - **La pastille s'explique au survol depuis LIVA #1151 (2026-09-11)** : `PrixTooltip` (même panneau Prix calculé / Prix de base / Retenu que le dialogue de ligne) sur `ligne.tarif`, calculé par ligne dans la liste des commandes **sous la règle qui a réellement tarifé la ligne** — `'price-floor'` pour un miroir (la base retenue nue, « Prix calculé » = coût / 0,7), `'cost-floor'` pour une ligne native — plus l'écart entre le prix saisi et la règle, et la marge contre le coût de revient à la quantité de la ligne. Le ticket : 2,30 € = le prix de base de la fiche 180 retenu tel quel (coût 1,16 € à 2 000 kg, marge 50 %), lu comme « 40 % de marge » sans rien à l'écran pour le dire. ⚠️ `retain()` comparait la base au coût **nu** sous `'price-floor'` : un miroir dont le coût tombe entre base × 0,7 et la base annonçait « base » en portant coût / 0,7 — corrigé, test dans `pricing-trm.test.ts`.
